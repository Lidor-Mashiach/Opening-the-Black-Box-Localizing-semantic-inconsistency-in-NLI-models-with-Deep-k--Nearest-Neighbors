...
temperature ramp per round, bounded by `max_generation_rounds`) -> **up to 5
verified paraphrases per hypothesis** (target 5, kept partial: a hypothesis
with 1-4 verified paraphrases is kept as-is - only a hypothesis with zero
...
Up to 5 verified paraphrases per hypothesis (kept partial, only zero-verified dropped), triple-gated. **Commit
...
```bash
# submit the nine tuning studies (the 3 large-model MNLI combos use official checkpoints, not tuned)
