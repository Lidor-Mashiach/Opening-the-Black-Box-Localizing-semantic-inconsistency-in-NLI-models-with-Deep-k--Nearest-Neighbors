"""Meta-runner for BERT-base x SNLI: clean workspace -> train -> filter -> encode -> evaluate.
...
if __name__ == "__main__":
    clean_combo_workspace(load_config("BERT-base", "SNLI"))
    for script in ORDER:
        banner("STEP-1", f"running {script}", "BERT-base", "SNLI")
        if subprocess.run([sys.executable, str(HERE / script)]).returncode != 0:
            sys.exit(f"FAILED: {script}")
    banner("STEP-1", "finished", "BERT-base", "SNLI")
