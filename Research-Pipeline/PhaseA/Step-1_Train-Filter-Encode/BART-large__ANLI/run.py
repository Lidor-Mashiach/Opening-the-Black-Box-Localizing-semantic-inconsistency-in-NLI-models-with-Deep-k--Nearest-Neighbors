"""Meta-runner for BART-large x ANLI: clean workspace -> train -> filter -> encode -> evaluate.
...
if __name__ == "__main__":
    clean_combo_workspace(load_config("BART-large", "ANLI"))
    for script in ORDER:
        banner("STEP-1", f"running {script}", "BART-large", "ANLI")
        if subprocess.run([sys.executable, str(HERE / script)]).returncode != 0:
            sys.exit(f"FAILED: {script}")
    banner("STEP-1", "finished", "BART-large", "ANLI")
