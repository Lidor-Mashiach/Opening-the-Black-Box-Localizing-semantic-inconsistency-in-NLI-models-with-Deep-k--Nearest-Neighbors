...
`MODEL_KEY = "DeBERTa-large"`, `DATASET_KEY = "MNLI"` and call it. Configuration comes from
...
|---|--------|--------------|--------|
| 1 | `train.py` | Fine-tune DeBERTa-large on the MNLI train split | `results/checkpoints/final/`, `results/train_metrics.json` |
| 2 | `build_filtered_dataset.py` | Keep only train rows the model got right | `Datasets/.../filtered/DeBERTa-large/train_correct.csv` |
| 3 | `encode_hypotheses.py` | Store every kept hypothesis at every layer | `Datasets/Encoded_Datasets/DeBERTa-large/MNLI/hypotheses.npz` |
...
`microsoft/deberta-large-mnli` (see `configs/models/DeBERTa-large.yaml`) -
