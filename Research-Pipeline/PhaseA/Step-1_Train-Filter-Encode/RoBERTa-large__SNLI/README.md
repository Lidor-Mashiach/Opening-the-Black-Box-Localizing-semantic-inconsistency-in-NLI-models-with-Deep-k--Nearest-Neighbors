# RoBERTa-large x SNLI - Step-1 Combination Folder

Thin, per-combination wrappers. All real logic lives in
[`Research-Pipeline/common/`](../../../common/README.md); these scripts only pin
`MODEL_KEY = "RoBERTa-large"`, `DATASET_KEY = "SNLI"` and call it. Configuration comes from
...
|---|--------|--------------|--------|
| 1 | `train.py` | Fine-tune RoBERTa-large on the SNLI train split | `results/checkpoints/final/`, `results/train_metrics.json` |
| 2 | `build_filtered_dataset.py` | Keep only train rows the model got right | `Datasets/.../filtered/RoBERTa-large/train_correct.csv` |
| 3 | `encode_hypotheses.py` | Store every kept hypothesis at every layer | `Datasets/Encoded_Datasets/RoBERTa-large/SNLI/hypotheses.npz` |
