"""Meta-runner for RoBERTa-large x MNLI: clean workspace -> train -> filter -> encode -> evaluate.
...
if __name__ == "__main__":
    clean_combo_workspace(load_config("RoBERTa-large", "MNLI"))
    for script in ORDER:
        banner("STEP-1", f"running {script}", "RoBERTa-large", "MNLI")
        if subprocess.run([sys.executable, str(HERE / script)]).returncode != 0:
            sys.exit(f"FAILED: {script}")
    banner("STEP-1", "finished", "RoBERTa-large", "MNLI")
