"""Meta-runner for DeBERTa-large x SNLI: clean workspace -> train -> filter -> encode -> evaluate.
...
if __name__ == "__main__":
    clean_combo_workspace(load_config("DeBERTa-large", "SNLI"))
    for script in ORDER:
        banner("STEP-1", f"running {script}", "DeBERTa-large", "SNLI")
        if subprocess.run([sys.executable, str(HERE / script)]).returncode != 0:
            sys.exit(f"FAILED: {script}")
    banner("STEP-1", "finished", "DeBERTa-large", "SNLI")
