...
`MODEL_KEY = "BART-large"`, `DATASET_KEY = "SNLI"` and call it. Configuration comes from
...
|---|--------|--------------|--------|
| 1 | `train.py` | Fine-tune BART-large on the SNLI train split | `results/checkpoints/final/`, `results/train_metrics.json` |
| 2 | `build_filtered_dataset.py` | Keep only train rows the model got right | `Datasets/.../filtered/BART-large/train_correct.csv` |
| 3 | `encode_hypotheses.py` | Store every kept hypothesis at every layer | `Datasets/Encoded_Datasets/BART-large/SNLI/hypotheses.npz` |
